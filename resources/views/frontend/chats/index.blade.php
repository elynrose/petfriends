        ...
        <div class="col-md-12">
            @can('chat_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.chats.create') }}">
                            ...
                            <tbody>
                                @foreach($chats as $key => $chat)
                                    <tr data-entry-id="{{ $chat->id }}">
                                        ...
                                        <td>
                                            @can('chat_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.chats.show', $chat->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('chat_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.chats.edit', $chat->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('chat_delete')
                                                <form action="{{ route('frontend.chats.destroy', $chat->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('chat_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.chats.massDestroy') }}",
