    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.chats.update", [$chat->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('booking') ? 'is-invalid' : '' }}" name="booking_id" id="booking_id" required>
                    @foreach($bookings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('booking_id') ? old('booking_id') : $chat->booking->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message">{!! old('message', $chat->message) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('from') ? 'is-invalid' : '' }}" name="from_id" id="from_id" required>
                    @foreach($froms as $id => $entry)
                        <option value="{{ $id }}" {{ (old('from_id') ? old('from_id') : $chat->from->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('from'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('read') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="read" id="read" value="1" {{ $chat->read || old('read', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="read">{{ trans('cruds.chat.fields.read') }}</label>
                </div>
                @if($errors->has('read'))
                    <div class="invalid-feedback">
                        {{ $errors->first('read') }}
