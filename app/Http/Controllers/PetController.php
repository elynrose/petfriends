    ...
    public function update(UpdatePetRequest $request, Pet $pet)
    {
        ...
        if (!$request->input('not_available')) {
            ...
            if (!$pet->not_available) {
                ...
                if ($hourDifference > 0) {
                    ...
                    if (!$user->hasEnoughCredits($hourDifference)) {
                        return redirect()->back()
                            ->with('error', "You need {$hourDifference} additional credits to extend this availability period. You currently have {$user->credits} credits.");
                    }
                    $user->deductCredits($hourDifference, "Extended pet availability by {$hourDifference} hours");
                } elseif ($hourDifference < 0) {
                    // Reducing availability - refund credits
                    $refundAmount = abs($hourDifference);
                    $user->refundCredits($refundAmount, "Reduced pet availability by {$refundAmount} hours");
                }
            } else {
                // Pet was not available before - charge full duration
                if (!$user->hasEnoughCredits($newHours)) {
                    return redirect()->back()
                        ->with('error', "You need {$newHours} credits to make your pet available for this duration. You currently have {$user->credits} credits.");
                }
                $user->deductCredits($newHours, "New pet availability for {$newHours} hours");
            }
        } else {
            ...
            if (!$pet->not_available) {
                ...
                // Refund credits
                $user = Auth::user();
                $user->refundCredits($oldHours, "Pet marked as not available");
            }
        }
